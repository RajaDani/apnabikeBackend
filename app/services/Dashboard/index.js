...
async function chartData(res) {
    let chart = await sequelize.query('SELECT sum(bookedbikes.total_amount)  as total , monthname(bookedbikes.booked_from) as monthly , year(bookedbikes.booked_from) as yearly from apnabike.bookedbikes group by month(bookedbikes.booked_from) , year(bookedbikes.booked_from)', {
...
module.exports = { chartData, dashboardData, latestTransactions }