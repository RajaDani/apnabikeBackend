...
async function userDashboardData(data, res) {
    ...
    let thisMonth = await sequelize.query('SELECT count(*) as total from bookedbikes where month(booked_till)= ' + month + ' and year(booked_till) = ' + year + ' and user_id = ' + userId + '', {
        type: QueryTypes.SELECT
    })

    if (totalorders || cancelledOrders || thisMonth) return { totalorders, cancelledOrders, thisMonth };
